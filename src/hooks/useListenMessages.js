...
//     }, [socket, setMessages, messages, setLastMessages, updateLastMessages]);
// }
...
const useListenMessages = (onNavigate) => {
    const { socket } = useSocketStore()
    const { messages, setMessages, updateLastMessages } = useConversation()
    ...
    const handleNewMessage = useCallback((newMessage) => {
        setMessages([...messagesRef.current, newMessage.lastMessage])
        ...
        updateLastMessages(prevLastMessages => {
            return prevLastMessages.map(msg => {
                if (msg.chatRoomId === newMessage.chatRoomId) {
                    return {
                        ...msg,
                        lastMessage: newMessage.lastMessage,
    ...
    }, [setMessages, updateLastMessages, onNavigate])
